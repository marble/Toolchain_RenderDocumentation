...
if exitcode == CONTINUE:
    ...
    settings_cfg_general = lookup(milestones, "settings_cfg", "general", default={})
    for k in ["copyright", "description", "project", "release", "t3author", "version"]:
    ...
    sitemap_files_singlehtml_count = milestones.get("sitemap_files_singlehtml_" "count")
    if sitemap_files_singlehtml_count is not None:
        R["sitemap_files_singlehtml_count"] = sitemap_files_singlehtml_count

    R["postprocessed_html_files"] = milestones.get("all_html_files_sanitized_count", 0)
...
if TheProjectResultBuildinfoResultJsonfile:
    result["MILESTONES"].append(
        {
            "TheProjectResultBuildinfoResultJsonfile": TheProjectResultBuildinfoResultJsonfile
        }
    )
