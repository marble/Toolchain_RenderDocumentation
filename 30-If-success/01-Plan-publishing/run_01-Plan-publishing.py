...
if exitcode == CONTINUE:
    ...
    relative_part_of_builddir = lookup(
        milestones, "relative_part_of_builddir", default=None
    )
    ...
    buildsettings_localization = lookup(
        milestones, "buildsettings", "localization", default=""
    )
...
if exitcode == CONTINUE:
    ...
    if len(relative_part_of_builddir_stripped.split("/")) < required_parts:
        loglist.append(
            "'%s' is shorter than the '%s' required parts"
...
if exitcode == CONTINUE:
    ...
    if build_singlehtml_folder:
        publish_dir_singlehtml_planned = (
            publish_dir_planned + "/" + os.path.split(build_singlehtml_folder)[1]
        )
    ...
    if pdf_dest_file:
        publish_file_pdf_planned = (
            publish_dir_pdf_planned + "/" + os.path.split(pdf_dest_file)[1]
        )
    ...
    for a, b, c in tuples:
        if b:
            some_milestones[a] = url_of_webroot + b[len(TheProjectWebroot) :] + c
