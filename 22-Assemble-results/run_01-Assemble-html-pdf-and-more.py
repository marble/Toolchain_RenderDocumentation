...
if exitcode == CONTINUE:
    ...
    if not (1 and build_html and build_html_folder and TheProject and version):
        exitcode = 22
...
if exitcode == CONTINUE:
    all_html_files_sanitized = lookup(
        milestones, "all_html_files_sanitized", default=None
    )
    all_singlehtml_files_sanitized = lookup(
        milestones, "all_singlehtml_files_sanitized", default=None
    )
    ...
    postprocessing_is_required = lookup(
        milestones, "postprocessing_is_required", default=None
    )
...
if exitcode == CONTINUE:
    ...
    if build_singlehtml and build_singlehtml_folder:
        shutil.move(
            build_singlehtml_folder, os.path.join(TheProjectResultVersion, "singlehtml")
        )
    ...
    if package_file:
        ...
        package_file_new_value = os.path.join(
            TheProjectResultPackages, os.path.split(package_file)[1]
        )
...
if TheProjectResultVersion:
    result["MILESTONES"].append({"TheProjectResultVersion": TheProjectResultVersion})
