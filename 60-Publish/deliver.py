...
if __name__ == "__main__":
    ...
    for publish_data in publish_data_list:
        ...
        dest_htaccess = os.path.join(
            destabspart, publish_data["publish_project_dir"], ".htaccess"
        )
        print(
            "ssh",
            "\\\n   ",
        ...
        if todo_update_stable_symlink:
            ...
            dest_stable = os.path.join(
                destabspart, publish_data["publish_project_dir"], "stable"
            )
            print("#", dest_stable)
            ...
            if CONTINUE:
                if existing_stable_target and not re.match(
                    "\d+\.\d+(\.\d+)*", existing_stable_target
                ):
            ...
            if CONTINUE:
                ...
                candidate = sorted(
                    [highest_existing_version, our_new_version],
                    cmp=version_cmp,
                    reverse=True,
                )[0]
                if candidate != our_new_version:
                    print(
                        "# Existing version is higher than our new one. Nothing to do."
                    )
                    CONTINUE = 0
