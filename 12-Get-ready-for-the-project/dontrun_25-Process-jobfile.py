...
if exitcode == CONTINUE:
    if not jobfile_abspath:
        if os.path.isabs(jobfile):
            ...
            jobfile_abspath = os.path.abspath(
                os.path.join(initial_working_dir, jobfile)
            )
        else:
            CONTINUE = -2
...
if exitcode == CONTINUE:
    ...
    if jobfile_data:
        ...
        force_rebuild_needed = frpa(
            "force_rebuild_needed",
            milestones.get("force_rebuild_needed"),
            ATNM,
            int,
            jobfile_data,
        )
        make_html = frpa(
            "make_html", milestones.get("make_html"), ATNM, int, jobfile_data
        )
        make_latex = frpa(
            "make_latex", milestones.get("make_latex"), ATNM, int, jobfile_data
        )
        make_pdf = frpa("make_pdf", milestones.get("make_pdf"), ATNM, int, jobfile_data)
        make_singlehtml = frpa(
            "make_singlehtml",
            milestones.get("make_singlehtml"),
            ATNM,
            int,
            jobfile_data,
        )
        oo_parser = frpa(
            "oo_parser", milestones.get("oo_parser"), ATNM, str, jobfile_data
        )
        rebuild_needed = frpa(
            "rebuild_needed", milestones.get("rebuild_needed"), ATNM, int, jobfile_data
        )
        replace_static_in_html = frpa(
            "replace_static_in_html",
