...
if exitcode == CONTINUE:
    ...
    if localization and not xx_XX.match(localization):
        loglist.append(
            "Bad buildsettings.localization. "
...
if exitcode == CONTINUE:
    extensions_builddir_relpath = lookup(
        facts, "tctconfig", configset, "extensions_builddir_relpath", default=None
    )
    drafts_builddir_relpath = lookup(
        facts, "tctconfig", configset, "drafts_builddir_relpath", default=None
    )
    webroot_abspath = lookup(
        facts, "tctconfig", configset, "webroot_abspath", default=None
    )
...
if exitcode == CONTINUE:
    ...
    if buildsettings["ter_extkey"]:
        ...
        buildsettings["package_key"] = (
            "typo3cms.extensions." + buildsettings["ter_extkey"]
        )  # 09 package_key
        ...
        if buildsettings["builddir"].lower() in ["", "typo3cms/project/0.0.0"]:
            ...
            if (
                ...
                and buildsettings["localization"] != "default"
            ):
    ...
    # when cloning a repository
        ...
        parts = buildsettings["giturl"].split(
            "/"
            ...
            if buildsettings["builddir"] in ["", "typo3cms/project/0.0.0"]:
                if repo_relpath_without_ext:
                    buildsettings["builddir"] = osp.join(
                        drafts_builddir_relpath,
                        repo_domain_slugified,
                        repo_relpath_without_ext,
                        "latest",
                    )
