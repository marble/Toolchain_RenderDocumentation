...
if exitcode == CONTINUE:
    ready_for_build = milestones.get('ready_for_build')
    loglist.append(['ready_for_build', ready_for_build])
    rebuild_needed = milestones.get('rebuild_needed')
    loglist.append(['rebuild_needed', rebuild_needed])
    included_files_check = milestones.get('included_files_check')
    loglist.append(['included_files_check', included_files_check])
...
if exitcode == CONTINUE:
    ...
    # by definition:
    latex_file = os.path.join(outdir, 'PROJECT.tex')
...
if exitcode == CONTINUE:
    result['MILESTONES'].append({
        'build_latex': 'success', 'latex_file': latex_file})
