...
class DataCollector(object):
    ...
    def start_body(self, tag, attrs=[]):
        ...
        if self.taginfo:
            if attrs:
                self.collect(".. <%s>   %r\n\n" % (tag, attrs), "verbatim")
        if 0 and "include content directory":
            s = "\n" ".. sectnum::\n" ".. contents::\n" "   :backlinks: top\n" "\n"
            self.collect(s, "verbatim")
    ...
    def stop_paragraph(self, tag, attrs=[]):
        ...
        if s:
            ...
            if "tryTextWrapper":
                s = NL.join(
                    TextWrapper(initial_indent="", subsequent_indent="").wrap(s)
                )
    ...
    def normdata_paragraph(self, data, src=None):
        ...
        lines = data.split()
        if (
            self.last_src == "entityref" or self.last_src == "charref"
        ) and data.startswith(" "):
            lines.insert(0, "")
        if data.endswith(" "):
            lines.append("")
            ...
            if s:
                if href:
                    ...
                    if self.sbuf.tell() and self.sbuf.getvalue()[-1] in WHITESPACECHARS:
                        spacer = ""
                    self.collect(
                        "%s`%s <%s>`_ " % (spacer, s, href), "verbatim", src="a"
                    )
                    used = True
                else:
                    self.collect("%s" % s, "verbatim", src="a")
            if not href:
                self.parent.unused_atags_without_href_log.append(
                    (tag, attrs, (self.parent.lineno, self.parent.offset))
                )
            elif not used:
                self.parent.unused_atags_with_href_log.append(
                    (tag, attrs, (self.parent.lineno, self.parent.offset))
                )
            ...
            if s:
                if self.verbatim[-1]:
                    ...
                    if MAIN.options["revealUnderlineTags"]:
                        s = s.strip()
                        self.collect(
                            u"%s[!%s] %s [/%s] " % (spacer, tag, s, tag), "verbatim"
                        )
                    else:
                        s = s.strip()
                        if s:
                            self.collect(
                                u"%s:underline:`%s` " % (spacer, s), "verbatim"
                            )
...
class MyHTMLParser(six.moves.html_parser.HTMLParser):
    def __init__(self, tagwriter=None, taginfo=0, tablesas="dl"):
        ...
        self.datacollector = DataCollector(
            parent=self, taginfo=taginfo, tablesas=tablesas
        )
    ...
    def handle_endtag(self, tag):
        ...
        if self.tagwriter:
            if 0:
                self.tagprinter(
                    "  " * self.taglevel,
                    "%s" % self.datacollector.debuginfo("sbufs"),
                    NL,
                )
        ...
        if not tag0 == tag:
            print()
            print(
                "ERROR at line %s, column %s: unbalanced tags in '%s'"
                % (self.lineno, self.offset, args.infile)
            )
    ...
    def log_font(self, tag, attrs=[]):
        ...
        stats[0] += 1
        if 0 and "lets not collect linenumbers here":
            stats[1].append(self.lineno)
...
def main(
    f1name, f2name, f3name=None, f4name=None, appendlog=0, taginfo=0, tablesas="dl"
):
    ...
    if f1name == "-":
        ...
        False
        and f1 is None
        and f1name.startswith("http://")
        or f1name.startswith("https://")
    ):
    ...
    try:
        ...
        if 1:
            blocktitle = (
                '"Unexpected data". This is data we got '
        ...
        if 1:
            f2.write(SNIPPETS.include_includes_txt)
        if 0:
...
def get_argparse_args():
    ...
    parser.add_argument(
        "--logfile",
        help="filename to receive parsing statistics",
        dest="logfile",
        default=None,
    )
    parser.add_argument(
        "--treefile",
        help="filename to receive a treelike structure of the input file as seen by the parser",
        dest="treefile",
        default=None,
    )
    parser.add_argument(
        "-a",
        "--append-log",
        help="append logfile data at then end of outfile. FLAG: 0|1. Default: 0.",
        dest="appendlog",
        default=0,
        type=int,
        choices=[0, 1],
    ...
    parser.add_argument(
        "--taginfo",
        help="include a description of html tags like head, body, style, script etc. as comment into the reST file. FLAG: 0|1. Default: 0.",
        dest="taginfo",
        default=0,
        type=int,
        choices=[0, 1],
    ...
    parser.add_argument(
        "--tables-as",
        help="write tables as 'definition lists' (dl) or as 't3-field-list-table' (flt). Default: dl",
        dest="tablesas",
        default="dl",
        choices=["dl", "flt", "t3flt"],
        metavar="dl|flt|t3flt",
    )
