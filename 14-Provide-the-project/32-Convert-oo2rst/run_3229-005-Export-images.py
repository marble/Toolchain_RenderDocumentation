...
def process_file(rstfile, tabletype):
    ...
    with open(rstfile, "rb") as f1, open(tmpfile, "wb") as f2:
        for line in f1:
            if line.startswith(".. |"):
                parts = line.split()
                if len(parts) == 4:
                    if parts[1].endswith("|") and parts[2] == "image::":
                        m = re.match(
                            "^data:image/(?P<ext>[a-z]+);(?P<coding>\w+),(?P<data>.+)$",
                            parts[3],
                        )
                        if m:
                            ext = m.groupdict()["ext"]
                            ...
                            fpath_image = save_image(
                                parts[1], ext, coding, data, destfolder, tabletype
                            )
                            if fpath_image:
...
if masterdoc_manual_005_rst_saved_images:
    result["MILESTONES"].append(
        {"masterdoc_manual_005_rst_saved_images": masterdoc_manual_005_rst_saved_images}
    )
