...
rebuild_needed_run_command = tct.deepget(
    facts, "run_command", "rebuild_needed", default=None
)
...
if exitcode == CONTINUE:
    ...
    if any(
        [
            rebuild_needed_because_of_change,
            rebuild_needed_because_of_age,
            rebuild_needed_run_command,
            rebuild_needed_tctconfig,
        ]
    ):
        rebuild_needed = 1
    else:
...
if rebuild_needed_because_of_age is not None:
    result["MILESTONES"].append(
        {"rebuild_needed_because_of_age": rebuild_needed_because_of_age}
    )

if rebuild_needed_because_of_change is not None:
    result["MILESTONES"].append(
        {"rebuild_needed_because_of_change": rebuild_needed_because_of_change}
    )

if rebuild_needed_run_command is not None:
    result["MILESTONES"].append(
        {"rebuild_needed_run_command": rebuild_needed_run_command}
    )
