...
if exitcode == CONTINUE:
    loglist.append(
        "Disabled in tool (run_53-Create-package.py "
...
if exitcode == CONTINUE:
    ...
    package_language = lookup(
        milestones, "buildsettings", "package_language", default=None
    )
...
if exitcode == CONTINUE:
    ...
    if 0 and "remove all font files":
        shutil.rmtree(ospj(dest_folder, "html", "_static", "fonts"), ignore_errors=True)
    ...
    theme_no_fonts_css = ospj(
        dest_folder, "html", "_static", "css", "theme-no-fonts.css"
    )
