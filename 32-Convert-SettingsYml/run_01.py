...
if 0:
    CONTINUE = -1
    loglist.append('This tool is disabled.')

if exitcode == CONTINUE:
    settingsyml_file = milestones.get('settingsyml_file', '')
    settingscfg_file = milestones.get('settingscfg_file', '')
    if settingscfg_file:
        loglist.append('Info: Nothing to do. Settings.cfg is present.')
        CONTINUE = -1

if exitcode == CONTINUE:
    if not settingsyml_file:
        loglist.append('Info: Nothing to do. Settings.yml is not present.')
        CONTINUE = -1
